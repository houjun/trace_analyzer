...
int read_radar(char *filename, TraceList **write_list, TraceList **read_list, AccessPattern **access_pattern, uint64_t *totalbytesrw)
{
    ...
    while(fgets(buf, MAX_LINE_LENGTH, fp)) {
        ...
        if(strcmp(buf,"HEADER\n")==0) {
            ...
            sscanf(buf, "%s %d %" SCNu64 "",filepath, &commsize, totalbytesrw);
            proc_num = commsize;

            while(proc_num--) {
                //PROCESS <MPI Rank> <n: number of trace entries>
                fgets(buf, MAX_LINE_LENGTH, fp);
                sscanf(buf, "%s %d %" SCNu64 "",processtmp, &tmp_record->mpirank, &nrecords);
                for(i=0; i < nrecords; i++) {

                    //read next n records
                    fgets(buf, MAX_LINE_LENGTH, fp);
                    //<Op string> <Time delta> <m: # of accesses>
                    sscanf(buf, "%d %s %lf %lf %" SCNu64 "",&hierarchy_info, operator, &tmp_record->startTime, &tmp_record->endTime, &mpairs);

                    for(j=0; j < mpairs; j++) {
                        fgets(buf, MAX_LINE_LENGTH, fp);
                        sscanf(buf, "%" SCNu64 ", %" SCNu64 "", &tmp_record->offset, &tmp_record->size);
...
int pattern_contig(TraceList **tracelist, AccessPattern **pattern_head, int mpirank)
{
    ...
    while(list_i != NULL && list_i->next != NULL) {
        ...
        uint64_t tmppos = list_i->offset;
        uint64_t tmpsize = list_i->size;
...
int pattern_fixed_stride(TraceList **tracelist, AccessPattern **pattern_head, int mpirank)
{
    ...
    while(list_i != NULL && list_i->next != NULL) {
        ...
        list_j = list_i->next;
        uint64_t stride_size = 2;
        uint64_t tmppos = 0;
        uint64_t tmp_stride_size = 0;
...
int merge_kd(AccessPattern **kdpattern_head, int k)
{
    ...
    uint64_t tmpstridesize = 0;
    uint64_t tmpstartpos = 0;
    int hasmerge = 0;
    int mergecnt = 0;
            ...
            while(pattern_j != NULL) {
                ...
                for(i = 0; i < pattern_i->k; i++) {
                    ...
                    if(!check_pattern_same(pattern_i,pattern_j) || pattern_j->reqSize != pattern_i->reqSize
                            ...
                            || ( pattern_j->startPos - pattern_i->startPos < pattern_i->strideSize[pattern_i->k-1])) {
                        //
                        checkflag = 1;
                        break;
                    }
                    ...
                    while(pattern_k != NULL) {
                        ...
                        for(i = 0; i < pattern_j->k; i++) {
                            ...
                            if(!check_pattern_same(pattern_j,pattern_k) || pattern_k->reqSize != pattern_j->reqSize
                                    ...
                                    || ( pattern_k->startPos - pattern_j->startPos < pattern_i->strideSize[pattern_i->k-1])) {
                                checkflag = 1;
                                break;
                            }
                        }
                ...
                uint64_t tmpstartpos = pattern_i->startPos;
