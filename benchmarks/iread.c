...
int main (int argc, char *argv[])
{
    ...
    double read_time_start = 0.0;
    double read_time_end = 0.0;

    double iread_time_start = 0.0;
    double iread_time_end = 0.0;
    double elapsed = 0.0;
    ...
    for(k = 0; k < mat_num; k++){
        ...
        if(compare_mode == 1){
                ...
                if(my_rank == 1)
                printf("Read start:%lf\t",read_time_start);
                ...
                elapsed = read_time_end - read_time_start;
                if(my_rank == 1)
                printf("Read finish:%lf\t Total Read time:%lf\t bandwidth:%lf\n",read_time_end, elapsed, myrows*(n+1)*sizeof(int)/(elapsed*1024*1024));


                iread_time_start = MPI_Wtime();
                if(my_rank == 1)
                printf("IRead start:%lf\t",iread_time_start);
                ...
                iread_time_end = MPI_Wtime();
                elapsed = iread_time_end - iread_time_start;
                if(my_rank == 1)
                printf("IRead finish:%lf\t Total Read time:%lf\n",iread_time_end, elapsed);
                        }
                        else{
                ...
                iread_time_start = MPI_Wtime();
                if(my_rank == 1)
                printf("Wait finish:%lf\t",iread_time_start);
                ...
                elapsed = iread_time_end - iread_time_start;
                if(my_rank == 1)
                printf("Memcpy finish:%lf\t Total Memcpy time:%lf\t bandwidth:%lf\n",iread_time_end, elapsed, myrows*(n+1)*sizeof(int)/(elapsed*1024*1024));
...
//                      printf("I/O time of %d round: %lf\n",k , io_time_end - io_time_start);
    ...
    if(my_rank == 0){
        printf("Total I/O time: %lf  bandwidth: %.2lf MB/s\n", io_time, n*(n+1)*4.0*mat_num/(io_time*1024*1024));
